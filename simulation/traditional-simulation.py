...
# --- CONFIGURATION: TRADITIONAL SYSTEM ---
USE_YOLO_DATA = True 
STATIC_GREEN_TIME = 30  # Fixed time for every green light (seconds)

# Default values
defaultGreen = {0:STATIC_GREEN_TIME, 1:STATIC_GREEN_TIME, 2:STATIC_GREEN_TIME, 3:STATIC_GREEN_TIME}
defaultRed = 150
defaultYellow = 5
...
# --- FUNCTION: READ TXT LIMITS (For fair comparison) ---
def get_vehicle_limits_from_txt():
    try:
        with open("traffic_data.txt", "r") as f:
            ...
            if len(lines) >= 8:
                ...
                p1_cars, p1_motos, p1_trucks, p1_tuktuks = 5, 5, 2, 2

            print(f"[DATA LOADED] Spawning exact same traffic as Smart Test.")
        ...
        print("[ERROR] Could not read traffic_data.txt. Using defaults.")
        return {'car': 100, 'bike': 100, 'truck': 100, 'bus': 100}

vehicle_limits = get_vehicle_limits_from_txt()

# --- FUNCTION: GENERATE PNG REPORT (STATIC VERSION) ---
def generate_final_report_image():
    print("Generating Static Report Image...")
    
    REPORT_BG = (240, 240, 240)
    BLACK = (0, 0, 0)
    RED = (150, 0, 0) # Red title for Static
    GREY = (200, 200, 200)
    ...
    title_text = title_font.render("Final Report (STATIC MODE)", True, RED)
    ...
    for i, data in enumerate(total_row_data):
        text = header_font.render(data, True, RED) 
        report_surface.blit(text, (start_x + sum(col_widths[:i]) + 10, current_y))

    timestamp = time.strftime("%Y%m%d-%H%M%S")
    filename = f"static_traffic_report_{timestamp}.png"
    pygame.image.save(report_surface, filename)
    print(f"[SUCCESS] Static Report saved as: {filename}")
...
# --- UPDATED VEHICLE CLASS (With Deadlock Fix) ---
...
def initialize():
    # Load STATIC time (defaultGreen is fixed at 30s)
...
# --- TRADITIONAL REPEAT LOOP (No Smart Logic) ---
def repeat():
    global currentGreen, currentYellow, nextGreen
    phaseDirections = {0: ['right', 'left'], 1: ['down', 'up']}
    
    # Just run the green timer. No Queue check. No Gap check.
...
def simTime():
    ...
    while(True):
        ...
        if(timeElapsed==simulationTime):
            showStats()
            generate_final_report_image() # Generate Static Report
            os._exit(1) 
...
class Main:
    ...
    pygame.display.set_caption("TRADITIONAL TRAFFIC SIMULATION (STATIC)")
    ...
    while True:
        ...
        if manualSpawnEnabled:
            ...
            instructions = [
                "TRADITIONAL MODE (STATIC)",
                f"Fixed Green Time: {STATIC_GREEN_TIME}s",
                "Input: traffic_data.txt (Same as Smart)",
